...
```
   ...
   Google Drive ──trigger──►  n8n workflow
                  ...
                  │ 7. Append to Sheet            │
                  │ 8. Send Gmail alert ◄── flagged only
                  └───────────────────────────────┘
                                  │
              ┌───────────────────┴───────────────────┐
              ▼                                       ▼
         Google Sheets                            Gmail
         Transactions +                           (per anomaly)
         Anomalies tabs
```

The Sheet is the system of record. Gmail is for real-time alerts.
...
> Example: Marketing in May 2026 totals €4,500 against a 6-month average of €795 → *"Marketing spend this month (€4500) is 5.7x the 6-month average (€795)"*. The €4,500 Facebook charge is the single largest contributor and therefore carries the flag; the rest of the Marketing rows in May (none, in this demo) would be silent.
...
Roughly half a cent per CSV processed, dominated by the Haiku categorization call. A typical 30-row file is one batched Haiku call (cents fractions) plus zero to a few Sonnet explanations (a fraction of a cent each). Gmail, Sheets, Drive, and n8n itself are free at this volume.
...
2026-05-04  Facebook            €4500.00
    [LOW   ] new_vendor       First time seeing vendor "Facebook"
...
5. Within a minute: five Gmail alerts, each quoting the actual numbers behind the flag.
...
- **No authentication.** Single-user system. The Drive folder, Sheet, and Gmail account are all owned by one account.
...
- **Vendor name drift across runs.** The AI categorizer cleans vendor names (e.g. "Facebook Ads" → "Facebook"), which can silently split one vendor's history across two names if the cleaning is inconsistent between runs. Rules 1 and 3 compare against the cleaned name, so a drift in output breaks the historical baseline.
...
- **Per-vendor charts in Gmail alerts.** Embed a 6-month amount-over-time sparkline for the flagged vendor directly in the alert email. Makes it immediately verifiable without clicking through to the Sheet.
...
- **Daily digest email.** A scheduled workflow that reads the past 24 hours of anomalies and sends a summary — useful once alert volume grows beyond a handful per day.
- **Weekly executive summary.** Pull the week's anomalies and category totals, send to Sonnet, email a 5-bullet summary.
...
- **Real-time alerts:** Gmail — one email per flagged transaction
